...
pub const REPO_NAME: &str = "intacct-cli";
...
/// The child's JSON carries its own `"skill":"intacct-cli"` self-identifier, needed when `skill
...
mod tests {
    ...
    fn drop_skill_name_removes_the_redundant_self_identifier() {
        let child_stdout = json!({"skill": "intacct-cli", "installed": true, "path": "/x"});
        assert_eq!(
