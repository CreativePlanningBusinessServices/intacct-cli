...
pub const EMBEDDED_SKILL: &str = include_str!("../../skills/intacct-cli/SKILL.md");
            ...
            if reason == "no Claude config dir" {
                eprintln!(
                    "skill not installed ({reason}); place it yourself with \
                     `intacct-cli skill install --dir <your skills dir>`"
                );
            }
...
pub fn resolve_target(
    ...
    match config_base {
        Some(base) if base_exists => {
            SkillTarget::Write(base.join("skills").join("intacct-cli").join(SKILL_FILE))
        }
...
fn skip_json(reason: &str) -> Value {
    json!({"skill": "intacct-cli", "installed": false, "reason": reason})
}

fn installed_json(path: &Path, installed: bool, reason: Option<&str>) -> Value {
    let mut result = json!({
        "skill": "intacct-cli", "path": path.display().to_string(), "installed": installed,
    });
    ...
    fn resolve_target_builds_the_conventional_path_under_an_existing_base() {
        ...
        match resolve_target(None, Some(base), true) {
            SkillTarget::Write(path) => {
                assert_eq!(
                    path,
                    PathBuf::from("/home/x/.claude/skills/intacct-cli/SKILL.md")
                )
            }
